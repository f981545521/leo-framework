package cn.acyou.leo.framework.util.excel;
...
public class ExcelUtil {
    ...
    public @interface Excel {
        // 字段标题名称或文件名称
        String value();
        // excel操作类型ExcelType   :   ALL表示全部操作，IMPORT只导入操作，EXPORT只导出操作
     ...
     * 获取实体类带有@Excel的属性
     */
    private static List<Field> getExcelList(Class<?> entity, String type) {
        List<Field> list = new ArrayList<>();
        Field[] fields = entity.getDeclaredFields();
        for (Field field : fields) {
            if (field.isAnnotationPresent(Excel.class)) {
                String fieldType = field.getAnnotation(Excel.class).type();
                if(fieldType.equals("ALL") || fieldType.equals(type)){
                    list.add(field);
                }
            }
        }
    ...
    public static void genTemplate(Class<?> entity, String sheetTitle) {
        XSSFWorkbook workbook = getCommon(sheetTitle, getExcelList(entity, "IMPORT"));
    ...
    public static <T> void exportExcel(Class<?> entity, List<T> list, String sheetTitle) {
        List<Field> fields = getExcelList(entity, "EXPORT");
    ...
    public static <T> List<T> importExcel(Class<T> entity, InputStream inputStream){
        ...
        List<String> fns = getFieldName(getExcelList(entity, "IMPORT"));
